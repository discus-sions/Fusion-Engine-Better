...
class WeekData {
	...
	public static function reloadWeekFiles(isStoryMode:Null<Bool> = false)
	{
		weeksList = [];
		weeksLoaded.clear();
		var directories:Array<String> = [Paths.getPsychPreloadPath()];
		var directories2:Array<String> = [Paths.getDataPsychPreloadPath()];
		var originalLength:Int = directories.length;

		var sexList:Array<String> = CoolUtil.coolTextFile(Paths.getDataPsychPreloadPath('storySonglist.json'));
		for (i in 0...sexList.length) {
			for (j in 0...directories.length) {
				var fileToCheck:String = directories2[j] + sexList[i] + '.json';
